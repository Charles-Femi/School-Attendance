...
def register_students():
    ...
    def save_students():
        """Saves the entered student names to the students.json file."""
        # Clean and filter empty lines. User can enter full names like "John David Smith" on one line.
        names = [name.strip() for name in text_box.get("1.0", tk.END).split("\n") if name.strip()]
        if names:
            with open(STUDENTS_FILE, "w") as f:
                json.dump(names, f)
            messagebox.showinfo("Saved", "Student list saved successfully.")
            win.destroy()
        else:
            messagebox.showwarning("No Students", "Please enter student names before saving.")

    win = tk.Toplevel()
    win.title("Register Students")
    win.geometry("400x400")
    win.configure(bg="#e6f2ff")

    tk.Label(win, text="Enter one student per line (e.g., John D. Smith):", font=("Arial", 12), bg="#e6f2ff").pack(
        pady=10)
    text_box = tk.Text(win, width=40, height=15)
    text_box.pack(pady=10)
    ...
    if os.path.exists(STUDENTS_FILE):
        try:
            with open(STUDENTS_FILE, 'r') as f:
                ...
                text_box.insert(tk.END, "\n".join(existing_students))
        except json.JSONDecodeError:
            messagebox.showwarning("File Error", "Could not read existing students file. It might be corrupted.")

    tk.Button(win, text="Save Students", command=save_students, bg="#4CAF50", fg="white").pack(pady=5)


# --- Dashboard ---
def launch_dashboard(role, current_user=None):
    """Launches the main dashboard based on the user's role."""
    dash = tk.Tk()
    dash.title("Dashboard")
    dash.geometry("600x500")
    dash.configure(bg="#e6f2ff")

    tk.Label(dash, text=f"Logged in as: {role.upper()}", font=("Arial", 14, "bold"), bg="#e6f2ff", fg="#003366").pack(
        pady=10)

    if role in ["admin", "developer"]:
        tk.Button(dash, text="Manage Admins", command=lambda: manage_admins(role, current_user), bg="#FF9800",
                  fg="white", font=("Arial", 11), relief="raised", bd=3).pack(pady=5)
        tk.Button(dash, text="Configure Email Settings", command=configure_email_settings, bg="#6A0DAD", fg="white",
                  font=("Arial", 11), relief="raised", bd=3).pack(pady=5)  # New button for email config
        tk.Button(dash, text="Manage Email Recipients", command=manage_recipients, bg="#FF00FF", fg="white",
                  font=("Arial", 11), relief="raised", bd=3).pack(pady=5)  # New button for recipients
    if role == "developer":
        tk.Button(dash, text="Advanced Settings", command=advanced_settings, bg="#607D8B", fg="white",
                  font=("Arial", 11), relief="raised", bd=3).pack(pady=5)

    tk.Button(dash, text="Register Students", command=register_students, bg="#4CAF50", fg="white",
              font=("Arial", 11), relief="raised", bd=3).pack(pady=5)
    tk.Button(dash, text="Mark Attendance", command=mark_attendance, bg="#2196F3", fg="white", font=("Arial", 11),
              relief="raised", bd=3).pack(pady=5)
    tk.Button(dash, text="View Attendance File", command=export_to_excel, bg="#009688", fg="white",
              font=("Arial", 11), relief="raised", bd=3).pack(pady=5)
    tk.Button(dash, text="Show Attendance File Location", command=lambda: open_file_location(RECORDS_EXCEL_FILE),
              bg="#795548", fg="white", font=("Arial", 11), relief="raised", bd=3).pack(pady=5)

    tk.Button(dash, text="Logout", command=dash.destroy, bg="#F44336", fg="white", font=("Arial", 11), relief="raised",
              bd=3).pack(pady=20)

    tk.Label(dash, text=CREATOR_NOTE, bg="#e6f2ff", fg="#003366", font=("Arial", 10, "bold")).place(relx=1.0, rely=1.0,
                                                                                                    anchor='se', x=-10,
                                                                                                    y=-10)

    # Start the attendance alert checker when the dashboard launches
    check_and_alert_teacher(dash)
    dash.mainloop()


# --- GUI Login Setup ---
# Initialize the application settings and files before launching the main window
initialize_app()
...
tk.Label(root, text=APP_TITLE, font=("Helvetica", 18, "bold"), bg="#B3E5FC", fg="#003366").pack(pady=30)
tk.Label(root, text="Username:", font=("Arial", 12), bg="#B3E5FC").pack()
username_entry = tk.Entry(root, width=30, font=("Arial", 12))
username_entry.pack(pady=5)

tk.Label(root, text="Password:", font=("Arial", 12), bg="#B3E5FC").pack()
password_entry = tk.Entry(root, show="*", width=30, font=("Arial", 12))
password_entry.pack(pady=5)

tk.Button(root, text="Login", command=login, bg="#0288D1", fg="white", font=("Arial", 12, "bold"), width=15,
          relief="raised", bd=3).pack(
    pady=30)
tk.Label(root, text=CREATOR_NOTE, bg="#B3E5FC", fg="#003366", font=("Arial", 10, "bold")).pack(side="bottom", pady=10)
